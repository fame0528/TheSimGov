/**
 * @file src/lib/db/models/Certification.ts
...
CertificationSchema.index({ code: 1 }, { unique: true }); // Unique certification codes
 ...
 * 
 * @description
 * Average revenue per certified professional (ARPU)
 * 
 * @returns {number} Revenue per certified ($)
 */
 ...
 * 
 * @description
 * Estimated annual renewal revenue
 * Assumes 70% renewal rate
 * 
 * @returns {number} Annual renewal revenue ($)
 */
 ...
 * 
 * @description
 * Total revenue from exam retakes
 * 
 * @returns {number} Retake revenue ($)
 */
 ...
 * 
 * @description
 * Total number of exam attempts (passed + failed)
 * 
 * @returns {number} Total exam takers
 */
...
CertificationSchema.pre<ICertification>('save', function (next) {
  ...
  if (this.jobPostings > 0) {
    // High demand: >10,000 postings = 90-100%, Medium: 1,000-10,000 = 60-90%, Low: <1,000 = 30-60%
 ...
 *   name: "Certified Cloud Architect",
 *   code: "CCA",
 *   description: "Professional certification for cloud architecture design and implementation",
 *   type: "Professional",
 *   examDuration: 180, // 3 hours
 *   questionCount: 75,
 *   passingScore: 70,
 *   examFormat: "Mixed",
 *   handsOnLabs: true,
 ...
 *   renewalFee: 150,
 *   examFee: 400,
 *   prerequisites: ["3 years cloud experience", "Associate certification"],
 ...
 * });
 * 
 * // Check certification health
 * console.log(cert.passRate); // e.g., 65% pass rate
 * console.log(cert.marketDemand); // e.g., 85% demand score
 * console.log(cert.renewalRevenue); // e.g., $105k annual renewals
 * console.log(cert.profitMargin); // e.g., 82% margin
 * ```
 */
