/**
 * @file src/lib/db/models/SponsorshipDeal.ts
 ...
 * - Deal value calculation: Upfront + (Monthly × Duration) + Expected bonuses
 * - Revenue share: Sponsor gets X% of ad revenue from sponsored content
 * - Performance bonuses: Triggered by impression/engagement thresholds
 * - Exclusivity premium: +30-50% deal value for competitor ban
 * - Brand lift measurement: Compare brand awareness before/after campaign
 * - Fulfillment rate: (Delivered / Required) × 100, penalties if <80%
 * - Dispute resolution: Mediation → Arbitration → Legal action escalation
 * - IP ownership: Content creator retains IP, sponsor gets perpetual usage rights
 */
 ...
 * Deal structure enum
 */
export type DealStructure = 'FlatFee' | 'RevenueShare' | 'PerformanceBased' | 'Hybrid';
...
/**
 * SponsorshipDeal document interface
 */
