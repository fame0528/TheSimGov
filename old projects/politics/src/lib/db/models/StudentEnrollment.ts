/**
 * @file src/lib/db/models/StudentEnrollment.ts
...
const StudentEnrollmentSchema = new Schema<IStudentEnrollment>(
  {
    ...
    startedAt: {
      type: Date,
      // Optional - set when student first accesses content
    },
    completionDate: {
      type: Date,
      // Optional - set when student completes course
      index: true,
    },
    ...
    expirationDate: {
      type: Date,
      // Optional - only for certifications with expiration
    },
 ...
 * 
 * @description
 * Number of lessons left to complete
 * 
 * @returns {number} Lessons remaining
 */
 ...
 * 
 * @description
 * Whether student is actively engaged (accessed within last 7 days)
 * 
 ...
 * Higher = more likely to drop out
 * 
 * Formula: 
 * - Base risk from inactivity (days since last activity)
 * - Reduced by progress made
 * - Increased by low engagement metrics
 * 
 * @returns {number} Dropout risk score (0-100)
 */
...
StudentEnrollmentSchema.pre<IStudentEnrollment>('save', function (next) {
  ...
  if (this.status === 'Completed' && this.certification && this.examPassed && !this.certificateIssued) {
    ...
    // Certificate number format: CERT-YYYY-ENROLLMENT_ID (will be set after save)
  }
 ...
 *   feedbackComments: "Excellent course! Highly recommend."
 * });
 * 
 * // Check enrollment health
 * console.log(enrollment.progress); // e.g., 68% complete
 * console.log(enrollment.lessonsRemaining); // e.g., 16 lessons
 * console.log(enrollment.isActive); // true if accessed within 7 days
 * console.log(enrollment.dropoutRisk); // e.g., 15% dropout risk
 * ```
 */
