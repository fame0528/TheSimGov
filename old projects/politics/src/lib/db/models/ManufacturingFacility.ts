/**
 * @file src/lib/db/models/ManufacturingFacility.ts
 ...
 * - roboticsCo
unt: Number of industrial robots
 ...
 *   company: companyId,
 *   name: "Detroit Assembly Plant #1",
 *   location: "Detroit, MI, USA",
 *   facilityType: 'Discrete',
 *   size: 500000,
 *   theoreticalCapacity: 10000,
 *   actualCapacity: 8500,
 ...
 * 
 * // Calculate OEE
 * facility.oeeScore = (facility.availability * facility.performance * facility.quality) / 10000;
 * await facility.save();
 * ```
 * 
 ...
 * - Planned downtime (preventive maintenance): 5-10% of total hours (40-80 hrs/month)
 * - Unplanned downtime (breakdowns): Target < 5% (< 40 hrs/month)
 * - Defect rate measured in PPM: Good (< 1000 PPM), Excellent (< 100 PPM), Six Sigma (< 3.4 PPM)
 * - Shift configurations: 1-shift (40hr/wk), 2-shift (80hr/wk), 3-shift (120hr/wk, 24/7 operation)
 * - Utilization rate = Actual / Theoretical capacity (70% = normal, 85% = good, 95% = excellent)
 * - Automation progression: Manual (1x) → Semi (1.5x) → Fully (2.5x) → Lights-Out (4x efficiency)
 * - Capital investment includes: land, building, equipment, initial inventory, automation systems
 * - Sustainability scoring: Energy efficiency, waste reduction, emissions control, water conservation
 * - ISO compliance: ISO 9001 (quality), ISO 14001 (environmental), ISO 45001 (safety)
 */
...
export type FacilityType =
  | 'Discrete'      // Unit-based production (automotive, electronics, machinery)
  | 'Process'       // Continuous/batch production (chemicals, food, pharma)
  | 'Assembly';     // Component assembly (furniture, appliances, consumer goods)

/**
 * Automation levels
 */
export type AutomationLevel =
  | 'Manual'        // Manual labor, minimal automation (1x efficiency)
  | 'SemiAuto'      // Partial automation (1.5x efficiency)
  | 'FullyAuto'     // Fully automated lines (2.5x efficiency)
  | 'LightsOut';    // Unmanned operation, AI-driven (4x efficiency)

/**
 * ISO compliance certifications
 */
export type ISOCompliance = 'ISO9001' | 'ISO14001' | 'ISO45001' | 'ISO50001';

/**
 * ManufacturingFacility document interface
 * 
 * @interface IManufacturingFacility
 * @extends {Document}
 */
...
const ManufacturingFacilitySchema = new Schema<IManufacturingFacility>(
  {
    ...
    facilityType: {
      ...
      enum: {
        values: ['Discrete', 'Process', 'Assembly'],
    ...
    automationLevel: {
      ...
      required: true,
      enum: {
        values: ['Manual', 'SemiAuto', 'FullyAuto', 'LightsOut'],
    ...
    isoCompliance: {
      type: [String],
      enum: ['ISO9001', 'ISO14001', 'ISO45001', 'ISO50001'],
      default: [],
    },
...
ManufacturingFacilitySchema.virtual('automationEfficiency').get(function (this: IManufacturingFacility): number {
  const automationMultipliers: Record<AutomationLevel, number> = {
    Manual: 1.0,
    SemiAuto: 1.5,
    FullyAuto: 2.5,
    LightsOut: 4.0,
 ...
 * const facility = await ManufacturingFacility.create({
 *   company: companyId,
 *   name: "Shanghai Electronics Plant",
 *   location: "Shanghai, China",
 *   facilityType: 'Discrete',
 *   size: 750000,
 *   theoreticalCapacity: 50000,
 *   actualCapacity: 42500,
 *   automationLevel: 'FullyAuto',
 *   automationScore: 85,
 *   roboticsCount: 150,
 *   productionLines: 12,
 *   shiftsPerDay: 3,
 *   capitalInvested: 50000000
 * });
 * 
 * // Find high-performing facilities
 * const topFacilities = await ManufacturingFacility.find({
 *   company: companyId,
 *   active: true,
 *   oeeScore: { $gte: 85 }
 * }).sort({ oeeScore: -1 });
