 ...
 *    - 2025 AI market rates calibrated from OpenAI, Anthropic, Google hiring data
 *    - PhD premium: 1.3x-1.8x depending on research intensity of role
 *    - Location multiplier: SF/NYC 1.45-1.50x, Seattle 1.35x, Austin 1.25x
 *    - Experience premium: Logarithmic growth (diminishing returns after 10 years)
 *    - Market conditions: AI boom (1.15x growth), high competition (1.2x multiplier)
 * 
 * 2. CANDIDATE GENERATION:
 *    - Skill tier distribution: 40% Junior, 35% Mid, 20% Senior, 5% PhD (10x rarer)
 *    - Reputation bonus: -10 to +10 skill points based on company reputation (50 = baseline)
 *    - PhD candidates: 2-5 papers/year, h-index ~60% of publications
 *    - University tier affects PhD quality (Stanford/MIT 20%, Cornell/UW 40%, Purdue/UMich 40%)
 * 
 * 3. RETENTION RISK:
 *    - Weighted factors: Salary gap (40%), Satisfaction (30%), Tenure (15%), External (15%)
 *    - Peak flight risk: 1-3 years in role (settling period over, not yet entrenched)
 *    - Critical threshold: 80+ risk score = actively job hunting
 *    - Counter-offer strategy: Close 70% of salary gap to market rate
 * 
 * 4. PRODUCTIVITY:
 *    - Formula: (researchAbility × codingSkill × computeResources) / projectComplexity
 *    - Research output: PhD researchers with high compute → 1-2 papers/month potential
 *    - Code output: Skill × resources / complexity (0-100 scale)
 *    - Optimal compute: $500/skill point (skill 10 = $5k/mo ideal)
 * 
 * 5. PROMOTION:
 *    - Tenure requirements: 2-2.5 years minimum (research roles slower)
 *    - Performance threshold: 7/10 consistently
 *    - Skill threshold: 75+ average skill
 *    - Company level gates: Senior roles need Level 3+, Principal needs Level 4-5
 *    - Research Scientists: 3+ publications required for promotion
 * 
 * 6. USAGE PATTERNS:
 *    - calculateCompetitiveSalary(): During hiring, annual reviews, counter-offers
 *    - generateCandidatePool(): Recruiting campaigns, hiring pipeline generation
 *    - calculateRetentionRisk(): Quarterly reviews, after external offers detected
 *    - calculateProductivity(): Project assignments, resource allocation
 *    - calculatePromotionEligibility(): Annual reviews, career planning discussions
 */
