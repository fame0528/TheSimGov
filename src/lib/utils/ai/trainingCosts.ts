...
import type { AIModelSize } from '@/lib/types/ai';
...
export function calculateIncrementalCost(
  ...
  const parameterFactor = Math.log10(parameters / 1_000_000_000 + 1);
 ...
 */
export function calculateCostPerEpoch(
  size: AIModelSize,
  parameters: number,
  datasetSize: number
): number {
...
export function estimateGPUHours(
  ...
  const billionParams = parameters / 1_000_000_000;
 ...
 */
export function calculateIncrementalCostWithCompute(
  size: AIModelSize,
  parameters: number,
  datasetSize: number,
  progressIncrement: number,
