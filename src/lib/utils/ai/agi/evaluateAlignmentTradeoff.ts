...
import { AlignmentStance, AlignmentTradeoffResult } from '@/lib/types/models/ai/agi';

/**
 * Input parameters for alignment tradeoff evaluation
 */
 ...
 */
export function evaluateAlignmentTradeoff(input: TradeoffInput): AlignmentTradeoffResult {
  ...
  const safetyFirstPath = {
    ...
    estimatedCost: 800_000_000, // $800M
    pros: [
  ...
  const balancedPath = {
    ...
    estimatedCost: 1_200_000_000, // $1.2B
    pros: [
  ...
  const capabilityFirstPath = {
    ...
    estimatedCost: 1_800_000_000, // $1.8B
  ...
  if (currentAlignment < 40) {
    ...
    recommendation = AlignmentStance.SAFETY_FIRST;
  ...
  } else if (currentAlignment >= 70 && availableBudget >= 1_800_000_000) {
    // Allow CapabilityFirst: High alignment + sufficient budget
    recommendation = AlignmentStance.CAPABILITY_FIRST;
  ...
  } else if (availableBudget < 1_000_000_000) {
    ...
    recommendation = AlignmentStance.BALANCED;
