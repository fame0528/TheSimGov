...
import type {
  PerformanceSnapshot as PerformanceDomain
} from '../../../types/media';

// Snapshot period enum (local to model - not part of domain types)
export type SnapshotPeriod = 'Daily' | 'Weekly' | 'Monthly' | 'AllTime';

// Traffic source interface (local extension)
...
// Extract domain shape excluding Mongoose-specific fields (domain is minimal snapshot, schema has full analytics)
type PerformanceBase = Omit<PerformanceDomain, 'timestamp' | 'performanceScore'>;

// Compose Mongoose document with domain base + ObjectId/Date overrides + full schema fields
interface ContentPerformanceDocument extends Document, PerformanceBase {
  ...
  // Performance Metrics (extends domain with additional fields)
  uniqueViewers: number;
  
  // Revenue Metrics (schema-specific)
  adRevenue: number;
  ...
  // Engagement Analysis (schema-specific)
  shareRate: number;
  completionRate: number;
  avgWatchTime: number;
  
  // Trend Analysis (schema-specific)
  viewsGrowth: number;
  revenueGrowth: number;
  engagementGrowth: number;
  rankVsOtherContent: number;
  
  // Audience Insights (schema-specific)
  topAgeGroup?: string;
...
export type IContentPerformance = ContentPerformanceDocument;
