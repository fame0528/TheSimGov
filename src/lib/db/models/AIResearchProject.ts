...
} from '@/lib/utils/ai/researchProjects';

/**
 * AIResearchProject Document Interface
 * Extends AIResearchProject type with Mongoose Document methods and virtuals
 */
export interface AIResearchProjectDocument extends Omit<AIResearchProject, 'id' | 'companyId' | 'assignedResearchers' | 'breakthroughs' | 'patents'>, Document {
  ...
  // Methods
  advanceProgress(increment: number, costIncurred: number, researcherSkills: number[]): Promise<void>;
 ...
 */
const aiResearchProjectSchema = new Schema<AIResearchProjectDocument>({
  company: {
  ...
  type: {
    type: String,
    required: true,
    enum: ['Performance', 'Efficiency', 'NewCapability'] as ResearchType[],
  },
  complexity: {
    type: String,
    required: true,
    enum: ['Low', 'Medium', 'High'] as ResearchComplexity[],
  },
  status: {
    type: String,
    required: true,
    default: 'InProgress',
    enum: ['InProgress', 'Completed', 'Cancelled'] as ResearchStatus[],
    index: true,
  },
  progress: {
    type: Number,
 ...
 */
aiResearchProjectSchema.virtual('activeResearchers').get(function(this: AIResearchProjectDocument) {
 ...
 * < 1.0 = over budget (bad)
 */
aiResearchProjectSchema.virtual('budgetEfficiency').get(function(this: AIResearchProjectDocument) {
  if (this.budgetSpent === 0) return 1.0;
 ...
 */
aiResearchProjectSchema.virtual('completionPercentage').get(function(this: AIResearchProjectDocument) {
 ...
 */
aiResearchProjectSchema.virtual('isInProgress').get(function(this: AIResearchProjectDocument) {
 ...
 */
aiResearchProjectSchema.virtual('isCompleted').get(function(this: AIResearchProjectDocument) {
...
aiResearchProjectSchema.methods.calculatePerformanceGain = function(
  this: AIResearchProjectDocument,
  researcherSkills: number[]
): PerformanceGain {
  return calculatePerformanceGain(
    this.type,
    this.complexity,
    researcherSkills,
 ...
 */
aiResearchProjectSchema.methods.advanceProgress = async function(
  this: AIResearchProjectDocument,
  increment: number,
  costIncurred: number,
  researcherSkills: number[]
  ...
  const gains = calculatePerformanceGain(
    this.type,
    this.complexity,
    researcherSkills,
    this.budgetAllocated,
 ...
 */
aiResearchProjectSchema.methods.cancel = async function(
  this: AIResearchProjectDocument,
  reason?: string
): Promise<void> {
 ...
 */
aiResearchProjectSchema.statics.getByCompany = async function(
  companyId: string | Types.ObjectId
): Promise<AIResearchProjectDocument[]> {
 ...
 */
aiResearchProjectSchema.statics.getByStatus = async function(
  companyId: string | Types.ObjectId,
 ...
 */
aiResearchProjectSchema.statics.getActive = async function(
  companyId: string | Types.ObjectId
): Promise<AIResearchProjectDocument[]> {
 ...
 */
aiResearchProjectSchema.pre('save', function(this: AIResearchProjectDocument, next) {
  // Validate budget overage (max 110% of allocated - uses utility)
  if (!validateBudgetOverage(this.budgetAllocated, this.budgetSpent)) {
    const error = new Error(
      `Budget overage exceeded: $${this.budgetSpent} spent vs $${this.budgetAllocated} allocated. ` +
  ...
  if (!validateResearcherCount(this.assignedResearchers.length)) {
    const error = new Error(
      `Invalid researcher count: ${this.assignedResearchers.length}. ` +
 ...
 * ```
 */
const AIResearchProjectModel: Model<AIResearchProjectDocument> = 
  mongoose.models.AIResearchProject || mongoose.model<AIResearchProjectDocument>('AIResearchProject', aiResearchProjectSchema);

export default AIResearchProjectModel;
