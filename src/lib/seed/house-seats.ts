/**
 * @fileoverview US House of Representatives Seat Structure
 * @module lib/seed/house-seats
 * 
 * OVERVIEW:
 * Complete structure of 436 US House seats (435 voting + 1 non-voting DC delegate)
 * Apportioned based on 2020 Census, representing 119th Congress (2025-2027)
 * 
 * Structure:
 * - 435 voting representatives (state apportionment by population)
...
/**
 * House seat configuration
 * Defines state, district number, and voting status
 */
export interface HouseSeatConfig {
  /** State abbreviation (e.g., 'CA', 'TX', 'NY') */
  state: StateAbbreviation;
  /** District number (0 for at-large, 1-N for numbered districts) */
  district: number;
  /** Voting status (false only for DC delegate) */
  isVoting: boolean;
}

/**
 * All 436 US House seats
 * Organized by state alphabetically, then by district number
 * Apportioned by 2020 Census population data
 */
export const allHouseSeats: readonly HouseSeatConfig[] = [
  // Alabama - 7 seats
  ...
  // California - 52 seats (largest delegation)
  ...
  // Texas - 38 seats (second largest delegation)
