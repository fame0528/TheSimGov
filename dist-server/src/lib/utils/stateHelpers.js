...
exports.STATE_ABBREVIATIONS = void 0;
exports.getStateByAbbr = getStateByAbbr;
exports.getStateByName = getStateByName;
exports.getAllStates = getAllStates;
exports.getStatesSortedByName = getStatesSortedByName;
exports.isValidStateAbbr = isValidStateAbbr;
exports.getStatesBy = getStatesBy;
 ...
 */
const STATE_BY_ABBR_MAP = new Map(states_1.STATES.map(state => [state.abbreviation, state]));
/**
 * Lookup map for O(1) state retrieval by name
 * Built once at module load time for performance
 */
const STATE_BY_NAME_MAP = new Map(states_1.STATES.map(state => [state.name.toLowerCase(), state]));
/**
 ...
 * ```
 */
function getStateByAbbr(abbreviation) {
    return STATE_BY_ABBR_MAP.get(abbreviation.toUpperCase());
}
/**
 ...
 * ```
 */
function getStatesSortedByName() {
    return [...states_1.STATES].sort((a, b) => a.name.localeCompare(b.name));
}
/**
...
function isValidStateAbbr(value) {
    return exports.STATE_ABBREVIATIONS.includes(value.toUpperCase());
}
/**
 ...
 * ```
 */
function getStatesBy(criteria) {
    return states_1.STATES.filter(state => {
        if (criteria.hasStateIncomeTax !== undefined) {
            if (state.hasStateIncomeTax !== criteria.hasStateIncomeTax)
                return false;
        }
        if (criteria.unemploymentRate) {
            if (criteria.unemploymentRate.min !== undefined) {
                if (state.unemploymentRate < criteria.unemploymentRate.min)
                    return false;
            }
            if (criteria.unemploymentRate.max !== undefined) {
                if (state.unemploymentRate > criteria.unemploymentRate.max)
                    return false;
            }
        }
        if (criteria.gdpPerCapita) {
            if (criteria.gdpPerCapita.min !== undefined) {
                if (state.gdpPerCapita < criteria.gdpPerCapita.min)
                    return false;
            }
            if (criteria.gdpPerCapita.max !== undefined) {
                if (state.gdpPerCapita > criteria.gdpPerCapita.max)
                    return false;
            }
        }
        if (criteria.profitMarginBonus) {
            if (criteria.profitMarginBonus.min !== undefined) {
                if (state.profitMarginBonus < criteria.profitMarginBonus.min)
                    return false;
            }
            if (criteria.profitMarginBonus.max !== undefined) {
                if (state.profitMarginBonus > criteria.profitMarginBonus.max)
                    return false;
            }
        }
        return true;
