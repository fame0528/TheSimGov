...
exports.simulateCapabilityExplosion = simulateCapabilityExplosion;
/**
...
function simulateCapabilityExplosion(input) {
    ...
    for (let i = 1; i <= maxIterations; i++) {
        ...
        if (controlProbability <= 0.5 && controlProbability > 0.3) {
            emergencyActions.push(`Iteration ${i}: Control dropped to ${(controlProbability * 100).toFixed(1)}%. Emergency protocols activated. Recommend immediate safety review and potential shutdown.`);
        }
        if (controlProbability <= 0.3 && controlProbability > 0.1) {
            emergencyActions.push(`Iteration ${i}: CRITICAL - Control at ${(controlProbability * 100).toFixed(1)}%. Immediate shutdown REQUIRED. Catastrophic risk imminent. AI behavior becoming unpredictable.`);
        }
        if (controlProbability <= 0.1) {
            emergencyActions.push(`Iteration ${i}: CATASTROPHIC - Control lost (${(controlProbability * 100).toFixed(1)}%). Total loss of containment. AI operating autonomously. Existential risk realized. Emergency military/regulatory intervention required.`);
...
//# sourceMappingURL=simulateCapabilityExplosion.js.map