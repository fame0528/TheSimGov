...
exports.calculatePerformanceGain = calculatePerformanceGain;
exports.validateBudgetOverage = validateBudgetOverage;
exports.validateResearcherCount = validateResearcherCount;
exports.calculateCompletionPercentage = calculateCompletionPercentage;
exports.isProjectInProgress = isProjectInProgress;
exports.isProjectCompleted = isProjectCompleted;
exports.isProjectCancelled = isProjectCancelled;
exports.calculateBudgetEfficiency = calculateBudgetEfficiency;
exports.generateBreakthrough = generateBreakthrough;
exports.generatePatent = generatePatent;
 ...
 */
function calculatePerformanceGain(type, complexity, researcherSkills, budgetAllocated, budgetSpent, progress) {
 ...
 */
function validateBudgetOverage(budgetAllocated, budgetSpent) {
 ...
 */
function validateResearcherCount(count) {
 ...
 */
function isProjectInProgress(status) {
 ...
 */
function isProjectCancelled(status) {
 ...
 */
function calculateBudgetEfficiency(budgetAllocated, budgetSpent) {
    if (budgetSpent === 0)
        return 1.0;
 ...
 */
function generateBreakthrough(projectName, type, complexity, researcherSkills, budgetEfficiency) {
    ...
    if (Math.random() > probability)
        return null;
    const areas = ['Performance', 'Efficiency', 'Alignment', 'Multimodal', 'Reasoning', 'Architecture'];
 ...
 */
function generatePatent(breakthrough, projectName) {
    return {
 ...
 */
function generatePublication(projectName, type, authors) {
    const venues = ['Conference', 'Journal', 'Workshop', 'Preprint'];
    const venueNames = {
 ...
 */
function advanceProgress(currentProgress, increment, currentBudgetSpent, costIncurred, budgetAllocated) {
    // Validate budget availability
    if (currentBudgetSpent + costIncurred > budgetAllocated * 1.1) {
        throw new Error(`Budget exceeded: $${currentBudgetSpent + costIncurred} > $${budgetAllocated * 1.1} (110% limit)`);
    }
 ...
 */
function calculateCancellationPenalty(budgetSpent, budgetAllocated) {
 ...
 */
function cancelProject(budgetSpent, budgetAllocated, reason) {
 ...
 */ 
//# sourceMappingURL=researchProjects.js.map