...
const AGIMilestoneSchema = new mongoose_1.Schema({
    company: {
    ...
    milestoneType: {
        type: String,
        enum: {
            values: Object.values(agi_1.MilestoneType),
    ...
    status: {
        type: String,
        enum: {
            values: Object.values(agi_1.MilestoneStatus),
            message: '{VALUE} is not a valid status',
        },
        default: agi_1.MilestoneStatus.LOCKED,
        index: true,
    },
    attemptCount: {
        type: Number,
    ...
    alignmentStance: {
        type: String,
        enum: {
            values: Object.values(agi_1.AlignmentStance),
            message: '{VALUE} is not a valid alignment stance',
        },
        default: agi_1.AlignmentStance.BALANCED,
    },
    alignmentChallenges: {
        type: [
            {
...
AGIMilestoneSchema.methods.calculateAchievementProbability = function () {
    return (0, ai_1.calculateAchievementProbability)(this.milestoneType, this.researchPointsInvested, this.currentCapability, this.currentAlignment);
};
/**
...
AGIMilestoneSchema.methods.evaluateAlignmentRisk = function () {
    return (0, ai_1.evaluateAlignmentRisk)(this.milestoneType, this.currentCapability, this.currentAlignment);
};
/**
...
AGIMilestoneSchema.methods.calculateImpactScore = function () {
    return (0, ai_1.calculateImpactScore)(this.currentCapability, this.currentAlignment, this.impactConsequences);
};
/**
...
AGIMilestoneSchema.methods.checkPrerequisites = function () {
    ...
    // );
    return (0, ai_1.checkPrerequisites)(this.researchRequirements, this.currentCapability, this.currentAlignment, this.researchPointsInvested, this.computeBudgetSpent, [] // Empty array for synchronous check - prerequisites assumed validated
    );
};
/**
 ...
 */
AGIMilestoneSchema.methods.attemptAchievement = async function (researchPoints, computeBudget) {
    ...
    if (success) {
        ...
        this.status = agi_1.MilestoneStatus.ACHIEVED;
        this.achievedAt = new Date();
        // Calculate capability gains (varies by milestone type)
        const capabilityGain = (0, ai_1.generateCapabilityGain)(this.milestoneType);
        const alignmentChange = (0, ai_1.generateAlignmentChange)(this.milestoneType);
        // Update current metrics
        this.currentCapability.reasoningScore = Math.min(100, this.currentCapability.reasoningScore + capabilityGain.reasoningScore);
        this.currentCapability.planningCapability = Math.min(100, this.currentCapability.planningCapability + capabilityGain.planningCapability);
        this.currentCapability.selfImprovementRate = Math.min(1, this.currentCapability.selfImprovementRate + capabilityGain.selfImprovementRate);
        this.currentCapability.generalizationAbility = Math.min(100, this.currentCapability.generalizationAbility + capabilityGain.generalizationAbility);
        this.currentCapability.creativityScore = Math.min(100, this.currentCapability.creativityScore + capabilityGain.creativityScore);
        this.currentCapability.learningEfficiency = Math.min(100, this.currentCapability.learningEfficiency + capabilityGain.learningEfficiency);
        // Update alignment
        this.currentAlignment.safetyMeasures = Math.max(0, Math.min(100, this.currentAlignment.safetyMeasures + alignmentChange.safetyMeasures));
        this.currentAlignment.valueAlignmentScore = Math.max(0, Math.min(100, this.currentAlignment.valueAlignmentScore + alignmentChange.valueAlignmentScore));
        this.currentAlignment.controlMechanisms = Math.max(0, Math.min(100, this.currentAlignment.controlMechanisms + alignmentChange.controlMechanisms));
        this.currentAlignment.interpretability = Math.max(0, Math.min(100, this.currentAlignment.interpretability + alignmentChange.interpretability));
        this.currentAlignment.robustness = Math.max(0, Math.min(100, this.currentAlignment.robustness + alignmentChange.robustness));
        this.currentAlignment.ethicalConstraints = Math.max(0, Math.min(100, this.currentAlignment.ethicalConstraints + alignmentChange.ethicalConstraints));
        // Calculate impact consequences
        const impact = (0, ai_1.calculateImpactConsequences)(this.milestoneType, this.currentCapability, this.currentAlignment);
        ...
        this.status = agi_1.MilestoneStatus.FAILED;
 ...
 */
AGIMilestoneSchema.statics.checkPrerequisitesAsync = async function (companyId, milestoneType) {
    ...
    if (prerequisiteTypes.length > 0) {
        const achievedPrerequisites = await AGIMilestone.find({
            ...
            status: agi_1.MilestoneStatus.ACHIEVED,
        }).select('milestoneType');
        const achievedTypes = achievedPrerequisites.map((m) => m.milestoneType);
        const missingPrerequisites = prerequisiteTypes.filter((prereq) => !achievedTypes.includes(prereq));
        // Calculate other requirements
        const capabilityMetrics = Object.values(milestone.currentCapability);
        const avgCapability = capabilityMetrics.reduce((s, v) => s + v, 0) / capabilityMetrics.length;
        const alignmentMetrics = Object.values(milestone.currentAlignment);
        const avgAlignment = alignmentMetrics.reduce((s, v) => s + v, 0) / alignmentMetrics.length;
...
AGIMilestoneSchema.pre('save', function (next) {
    ...
    if (this.status === agi_1.MilestoneStatus.ACHIEVED && !this.achievedAt) {
        this.achievedAt = new Date();
    }
    // Ensure failed milestones have failure date
    if (this.status === agi_1.MilestoneStatus.FAILED && !this.failedAt) {
        this.failedAt = new Date();
    }
    // Reset failure date if status changes from Failed
    if (this.isModified('status') && this.status !== agi_1.MilestoneStatus.FAILED) {
        this.failedAt = undefined;
    }
    next();
});
// Export model
const AGIMilestone = mongoose_1.default.models.AGIMilestone || mongoose_1.default.model('AGIMilestone', AGIMilestoneSchema);
exports.default = AGIMilestone;
/**
 ...
 */
//# sourceMappingURL=AGIMilestone.js.map