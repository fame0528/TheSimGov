 ...
 */
const TIER_UPTIME_REQUIREMENTS = {
...
const DataCenterSchema = new mongoose_1.Schema({
    company: {
 ...
 */
DataCenterSchema.methods.calculatePUE = function () {
...
DataCenterSchema.methods.checkTierRequirements = function () {
    ...
    if (this.powerRedundancy.generators < requirements.generators) {
        issues.push(`Tier ${tier} requires ${requirements.generators} generator(s) ` +
 ...
 */
DataCenterSchema.methods.estimateMonthlyPowerCost = function (powerCostPerKWh) {
    // Convert MW to kW
 ...
 */
DataCenterSchema.methods.addCertification = async function (type, cost) {
 ...
 */
DataCenterSchema.methods.recordDowntime = function (hours) {
...
DataCenterSchema.virtual('tier').get(function () {
    return this.tierCertification;
});
DataCenterSchema.virtual('powerUsageMW').get(function () {
    return this.powerUtilizationMW;
});
DataCenterSchema.virtual('currentPUE').get(function () {
    return this.pue;
});
DataCenterSchema.virtual('backupGenerators').get(function () {
    return this.powerRedundancy.generators;
});
DataCenterSchema.virtual('upsCapacity').get(function () {
    return this.powerRedundancy.ups ? this.coolingCapacityKW * 0.1 : 0; // Estimate 10% of cooling capacity
});
// Export model
const DataCenter = mongoose_1.default.models.DataCenter || mongoose_1.default.model('DataCenter', DataCenterSchema);
