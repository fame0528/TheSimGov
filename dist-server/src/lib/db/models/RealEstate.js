...
exports.PERMIT_TIMELINES = exports.ZONING_COMPLIANCE = exports.BUILDOUT_MULTIPLIERS = void 0;
const mongoose_1 = __importStar(require("mongoose"));
/**
 * Property type cost multipliers for buildout
 * Used by infrastructure utilities for cost estimation
 */
exports.BUILDOUT_MULTIPLIERS = {
 ...
 */
RealEstateSchema.methods.checkZoningCompliance = function (intendedUse) {
    const issues = [];
    // Check if use is permitted in current zoning classification
    const permittedZones = exports.ZONING_COMPLIANCE[intendedUse.toLowerCase()] || [];
    const zonePermitsUse = permittedZones.includes(this.zoneClassification);
    if (!zonePermitsUse) {
        issues.push(`${intendedUse} not permitted in ${this.zoneClassification} zones. ` +
 ...
 */
RealEstateSchema.methods.applyForPermit = async function (permitType) {
    ...
    const timelineDays = exports.PERMIT_TIMELINES[permitType] || 60; // Default 60 days
    const baseCost = 5000; // Base application fee
    // Property type affects cost (urban more expensive)
    const propertyMultiplier = exports.BUILDOUT_MULTIPLIERS[this.propertyType];
...
// Export model
const RealEstate = mongoose_1.default.models.RealEstate || mongoose_1.default.model('RealEstate', RealEstateSchema);
