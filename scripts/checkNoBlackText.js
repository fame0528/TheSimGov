...
const patterns = [
  { name: 'Tailwind token: text-default-700', re: /\btext-black\b/i },
  { name: 'Problem token: text-default-700', re: /\btext-default-500\b/i },
  { name: 'Hex black: #000 or #000000', re: /#(?:000|000000)\b/i },
  { name: 'rgb(0,0,0)', re: /rgb\(\s*0\s*,\s*0\s*,\s*0\s*\)/i },
  { name: 'color: var(--color-fg-default)', re: /color\s*:\s*(?:'|\")?\s*black\s*(?:'|\")?/i },
...
function walk(dir, results = []) {
  ...
  for (const ent of entries) {
    ...
    // Exclude legacy old projects/politics files
    if (fullPath.replace(/\\/g, '/').includes('old projects/politics/')) continue;
    if (ent.isDirectory()) {
      if (ignoreDirs.has(ent.name) || ent.name === 'scripts') continue;
